...
<br> ---Update At 20191029
<br>不常用但是出现过的API：
<br>select(index,....):
<br>A. index 是数字,那么返回参数中第 index 个之后的部分
<br>B. index 不是数字那就必须只能是字符串"#"，此时 select 返回参数的个数。

<br>数据类型：
<br>1. String
<br>2. Table 表：Lua表分为数组和散列部分。
<br>   A.使用Lua表需要注意的几点： 1. 尽量不要在一个表中混用数组和散列通部分，即一个表最好只存放一类数据。Lua上显示确实提供了两者
<br>统一的表示的遍历，但是这并不意味这使用者就应该混用这两种方式。
<br>   B.尽量不要在表中存放nil，这会让取长度操作的行为不稳定。
<br>   C.尽量避免重新散列操作，因为这个操作的代价极大，通过预分配。只使用数组部分等策略规避这个lua解析器背后的动作，能提升不少效率。
